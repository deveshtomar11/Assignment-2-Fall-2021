    ...
    class Program
    {
        ...
        */
        ...
        public static int LargestAltitude(int[] gain)
        {
            try
            {
                int max = 0;
                int current = 0;
                if ((gain.Length >= 1 && gain.Length <= 100) && (gain[i] >= -100 && gain[i] <= 100))
                {
        ...
        */
        ...
        public static int MaximumSum(int[] arr)
        {
            try
            {
                ...
                int sum = 0;
                if ((n >= 1 && n <= 3 * 10000) && (arr[i] >= -105 && arr[i] <= 105))
                {
                    for (int i = 0; i < n; i++)
                    {
                        sum += arr[i];
        ...
        */
        ...
        public static int minSubArrayLen(int target_subarray_sum, int[] arr10)
        {
            try
            {
                int minSubArrayLen = 0;                       //Result
                int runningSum = 0;                           //Store sum from subArrayStartIndex till i.
                int subArrayStartIndex = 0;
                for (int i = 0; i < arr10.Length; i++)
                {
                    runningSum += arr10[i];                    //keep incrementing runningSum until runningSum >= target_subarray_sum.
                    while (runningSum >= target_subarray_sum)                   //repeat this until runningSum >= target_subarray_sum since we need 'min'SubArrayLen.
                    {
                        if (minSubArrayLen == 0)                               //If this is the first minSubArray.
                            minSubArrayLen = (i - subArrayStartIndex) + 1;
                        else                                                   //We already found one minSubArray before.
                            minSubArrayLen = Math.Min(minSubArrayLen, (i - subArrayStartIndex) + 1);

                        runningSum = runningSum - arr10[subArrayStartIndex];     //Reduce runningSum by arr10[subArrayStartIndex]
                        subArrayStartIndex++;                           //Move subArrayStartIndex ahead since its not accounting for runningSum.
                    }
                }
